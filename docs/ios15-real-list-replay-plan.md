...
## Device crash 2026-09-23 and probe-only attach stripping

- First 5be66f70 device attempt crashed ~16s after launch (incident5AF7BB30, iOS15.1.1), SIGTRAP from `_assertionFailure` in `AIChatViewModel.minisAppGroupRoot` reached through AsyncImageTile body → minisMediaCacheKey → resolveMinisFileURL. The probe bundle intentionally has no normal-Minis App Group, so attachment tiles must not render. This is a harness defect; the production renderer path is untouched.
- Import now counts then clears every ChatMessage's attachment metadata after the real toChatMessage conversion. The displayed text is identical (the XML is stripped by that production converter), no attachment tile/URL resolution occurs, and report limits name the omission. No App Group entitlement or normal-Minis container access is added.
- Rebuild gate remains: exact inverse overlay, real window/callback native logs with both compiling mutations rejected, complete App build, package/signature/Mach-O/source-hash verification, then a fresh device run.
