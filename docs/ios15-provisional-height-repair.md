...
### Native policy cycle 2

- Run`35943869291`, commit`0af4773713a9d32b60f39c285b08b20ee1968938`: the admission regression is GREEN. Six controls pass; four independently executed cancellation assertions are RED (stable, post-thaw stable, accepted growth, growth inside the existing2pt deadband). Baseline compiled separately and produced its exact five expected failures.
- The second production slice cancels a pending value at the start of each fresh preferred-attributes observation, before either the deferred branch or early-return thresholds; only a current deferred shrink is requeued. This addresses the real Coordinator order (thaw → corrections → flush), not just dragging.
- New lifecycle assertions now exercise reset/invalidate/direct writes/confirmed-height authority, two-row swaps and removal, changed-vs-unchanged keys, width transition before debounce, and a scheduled width purge surviving reset. They must fail before the corresponding lifecycle edits. Existing renderer/host/cell code remains unchanged.

### Native policy cycle 3 and lifecycle implementation
