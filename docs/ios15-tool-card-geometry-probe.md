...
Before that larger build, run a much smaller **component contract** on the real Apple runtime: the actual `LegacyHostingContentView.contentSizeChanged` method is fed single, duplicate, grow/shrink-in-one-turn and root-replacement samples. The first baseline run was red on the real simulator: controls passed, but shrink/grow delivered the first queued height and root replacement delivered the old height through the new callback. The production candidate now retains the latest pending sample and tags queued delivery by configuration generation. Test samples48/72/36/96 are synthetic inputs, not injected screenshot row frames or replayed1161→36 layout values.
...
Important: the baseline red is a real component notification defect and a plausible mechanism for stale collection-row height, but it does **not alone prove it caused the screenshot**. A green result after the candidate patch is not UI acceptance. The production change is limited to the legacy size-notification boundary; it does not change the ToolCapsuleView's36pt design, collection spacing, preview/inset, or Retry behavior.
...
Checkpoint: baseline native evidence is captured in run `35547693286` (Xcode26.2/17C52, simulator iOS26.2): controls passed; all 12 non-control repetitions failed with stale-first-sample/old-generation delivery. Candidate source patch and contract-test updates are local and must rerun the same workflow before any IPA or device claim. No package has been built or delivered.
