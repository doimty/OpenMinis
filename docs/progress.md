...
## 2026-09-20 — new draft-thumbnail evidence; production geometry probe

- Superseding correction: the historical100pt subtraction and claims of full production Retry acceptance above are invalid, as already documented in `ios15-retry-hit-investigation.md`. Do not promote them back to root-cause facts. Screenshot dimensions are pixels, SwiftUI dimensions are points.
- New evidence: the draft image is almost completely hidden in the input composer; a second full screenshot shows the lower message/tool-preview overlap. New log hash `5ad0659b341bdb01385c9d40190099770470ca8279bd613f8b8caa42e068e9b1` records a65pt toolbar contribution, input bar115.67, adjusted inset188, bounded markdown widths and zero layer rejections. Two `继续` entries are ordinary composer sends, not orange Resume callbacks. No device build SHA in this log fragment.
- Live baseline `01d84edbfaaf23c108f843ce5e1762951c8d1369` equals fork's `fix/ios15-cell-hit-region`; cloud run35487147840 succeeded. This does not close device acceptance. No new IPA is delivered for the new attachment report.
- Created diagnostic branch `diagnostics/ios15-composer-layout`. Plan: `docs/ios15-composer-layout-investigation.md`. No production files changed. A separate pinned Xcode26.2/iOS26.2 workflow compiles source-derived actual legacy cell/layout and attachment composer paths, observes real frames/cache, and retains failed screenshots/reports.
- Attachment matrix includes actual LegacyFlowLayout, actual InputAttachmentGridView wrapper, actual CompatGeometry and pre26 ComposerSurface; native-flow/native-geometry ablations, positive-seed control, zero-grid negative oracle, add/remove/re-add/narrow cases. Fixed64 image chips substitute for image I/O, explicitly not a full photo-picker acceptance test.
- Collection matrix uses actual SelfSizingCell, NoAnimationCollectionView, MessageListViewController, MessageListLayout and legacy host. The test does not inject measured heights or manually invoke PLAF. Initial/grow/shrink must converge and not overlap adjacent rows.
- Local extraction contracts3/3, prior compatibility18/18 and retry4/4, Python/shell syntax and whitespace checks pass. Apple compile/runtime still pending at this checkpoint. Independent report `reports/openminis-ios15/attachment-layout-investigation-01d84ed.md` confirms the test seam, not a root cause. A green forced-legacy probe must be reported as not reproduced when it cannot catch the original device failure.
