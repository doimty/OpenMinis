...
## 2026-09-22 — device diagnostic delivered; provisional-height writes captured

- Full device run `35699475162` succeeded at source `2f21e242df71d63576682f8ac61c50a097f3a5ae`. Native collector checks, nonempty Xcode26.2/17C52 logs, IPA CRC/SHA/metadata and 14 Mach-O images verified. The 84,308,511-byte diagnostic IPA (SHA256 `ca90f97f935db020fd11245e7d3973ce8e18f0d45593592ba22f8cd91c8d43b7`) was sent through the native channel attachment tool. This is diagnostic delivery, not a production fix or UI acceptance.
- Source-matching real-device trace SHA256 `b6d0f8125139527a7e696951eafbc245f867f5583e2ef9af3f7499f945057134`: 8001 intact records / 13.599 seconds, then explicit cap. Target row 249 writes 1865→1484→1865 (191.913 ms, recovery is a different cell); row 253 writes 1521→1354→1521 (14.188 ms, same cell/configuration). The latter also observes actual cell frame heights 1354 then 1521. These intervals are cache-write intervals, not claimed screen-presentation durations.
- Parent/subject/cell-generation tracing connects unset-width text estimates through same-root host delivery, cell acceptance, preferred-height return and layout cache write. The correct finite-width text height was already computed before each bad delivery. Outer host width 428 does not attest that the descendant had valid width when producing its estimate. Existing generations are not mixed up or rolled back.
- Whole-file analyzer/hardware-loop status stays `INCONCLUSIVE` / exit1: cap occurs after switching to another session but before a recorded return to the original; user visual observation was not supplied. Local witness verifier confirms both chains and rejects 10 owner/parent/host/generation negative controls; it is explicitly an evidence checker, not a native candidate regression test.
- Detailed findings and source owners: `docs/ios15-reentry-device-capture-2f21e24.md`. 85.05% of records are host-size events; increasing root generations with reset `lastSize` defeat unchanged-size deduplication. The trace does not prove which setter callers or visibility states cause every repetition.
- No new commit, push or IPA has been made yet from this analysis. Production worktree changes now implement the narrow next candidate: `LegacyHostedMeasurementReadiness` is opt-in for `SelectableMarkdownTextView`; the host rejects provisional preferences until the text view has a finite bounds/container width and finite `sizeThatFits` token. A readiness callback advances a revision-stamped `LegacyHostedSize`; delayed old-revision preferences and queued deliveries are rejected before `lastSize`/`pendingSize` mutation. Plain SwiftUI content and attachment height growth remain allowed. Added Python contract checks plus a native XCTest source for unset→finite and width-change invalidation; the device workflow runs the contract and `build-for-testing` for `MinisTests`.
- Local checks after the candidate: readiness/width/compatibility contract suite 22/22, `git diff --check` passed. Linux cannot run Swift/UIKit. Cloud build and device re-test are still required; do not call this candidate fixed until the same phone/session no longer shows the jump and controls remain green.
