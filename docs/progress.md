...
## 2026-09-22 — cached session re-entry scroll-jump fix in progress

- Latest runtime log `/root/.openclaw/media/inbound/minis-2026-09-22---5ffe8fe9-6b8e-4d4e-995e-035e3b94fd91` shows 4925 browsing at `offset=24827`, `contentSize=25883`, `mode=browse`, then the same cached session remounts and emits `[forceScrollToBottom]` at 09:58:13, 09:58:20 and 09:58:22. During the browsing window there is no `SettleJitter`, post-settle clamp, or content-size shrink/grow evidence. This pins an unconditional cached-session re-entry scroll as the direct jump mechanism; it does not prove a duplicate row tap or a NavigationPath pop/push race.
- Working tree `fix/ios15-session-width-contract` now preserves a semantic message-list anchor (`MessageListScrollAnchor`) and explicit `messageListWasBrowsing` intent on `AIChatViewModel`. `CollectionViewMessageListV3.Coordinator` captures the first visible item plus its viewport-relative Y during scrolling, inherits browse mode before applying a replacement snapshot, and restores the anchor after layout. `AIChatView.onAppear` only sends the cached re-entry force-scroll when the session was not in browse mode.
- Added `scripts/test_ios15_session_reentry_scroll_contract.py`. Width contract, re-entry contract, and existing iOS 15 compatibility contract pass: 21 tests total; `git diff --check` passes. No Apple toolchain is installed in this workspace, so Swift/Xcode compilation and device acceptance remain pending. Do not claim the fix is device-verified until a fresh cloud IPA is installed and the same browse → leave → re-enter sequence is exercised.
- Independent issue remains: `3C88` references missing `minis://workspace/pelican-scene/scene-portrait.png`; `rawW=10000000` is only a TextKit unconstrained probe input and the current attachment fallback returns at most 400×200. Do not conflate this missing-file retry chain with the session scroll fix.

## 2026-09-22 — correction: rejected re-entry candidate retired, native acceptance reset

- This entry supersedes the preceding re-entry section's causal/fix claims. Review found 2 P1 verification defects and 3 P1 + 1 P2 behavioral/spec defects. The original two new tests passed even with both capture and restore bodies emptied in memory; a new Swift log statement also failed grammar parsing. Latest log chronology did not prove that force-scroll was the original single-session layout-jump cause.
- On the user's follow-up asking how to repair, archived the entire rejected diff plus all six affected files under workspace `reports/openminis-ios15/reentry-contract-review/rejected-candidate/`. Archive manifest verified against diff SHA256 `4a473f83b81cb393ce090eeecf9d3d9097103a73a2da008093228bd6a4b70d28` before any edits.
- Removed ONLY that candidate with targeted edits. All four Swift files now byte-match `git show ef9decf:<path>`. The rejected test was moved recoverably out of active scripts; its original content remains archived. Width + compatibility checks pass 19/19 and `git diff --check` passes. No commit/push/build/new package; this is verified retirement, not a claim the remaining device symptom is fixed.
- Replacement diagnostic/acceptance contract: `docs/ios15-session-reentry-validation.md`. Keep normal initial positioning and existing width/height fixes. Use the real parser/text view/hosting/cell/layout in a controlled re-entry case; correlate geometry and action sources; distinguish BUG_REPRODUCED, CANDIDATE_PASSES, INCONCLUSIVE and INVALID_RUN. No extra request for the already-provided original conversation.
- The existing `ios15-session-width-probe` is a micro width-contract comparison with a substitute text view; its `shrink` phase changes input and width. It is not a faithful unchanged-content re-entry reproduction. Linux cannot execute UIKit/Xcode; a new diagnostic branch commit/push/cloud run needs explicit approval. Private conversation contents must not be uploaded publicly.

## 2026-09-22 — diagnostic submission approved; simulator execution has a native dependency blocker

- User authorized the diagnostic commit/push/cloud-test scope. Created `diagnostics/ios15-session-reentry` from `ef9decf`; production source remains byte-identical to that baseline. The submission contains the corrected validation contract and this rejection/retirement record, not the rejected scroll-anchor implementation or private conversation fixtures.
- Preflight discovered that the preferred full-App UI-test route cannot currently link on Simulator: `libish_emu.a` is device-only, explicitly documented in `BUILDING.md` and `deps/build_rclone_ios.sh:8–11`. A pinned macOS/Xcode runner is not sufficient. Do not start the old width probe, build a device IPA, or label static checks a re-entry test to conceal this blocker.
- Width + compatibility checks passed 19/19 and `git diff --check` passed before the diagnostic documentation submission. Actual native re-entry execution remains blocked until a real standalone rendering/cell/layout target or a valid simulator dependency set exists. There is no new native pass, device acceptance, production fix, or package to deliver.

## 2026-09-22 — proceed through real-device diagnostics instead of blocking on Simulator

- User asked to continue toward compilation. The Simulator-specific iSH limitation does not block the already-working real-device build. Chosen path: bounded observation-only diagnostic IPA on the full production renderer/cell/layout path, then a structured hardware reproduction using the existing conversation. No new scroll-position feature and no speculative layout fix.
- Added opt-in `ReentryDiagnostics` scalar collector and observation blocks in the seven UI/logging files. Each event carries source SHA, run/sequence, anonymous object links and generation; no raw Markdown, image, provider credential or URL is recorded. A serial utility queue writes at most 8000 events plus a cap marker into a dedicated `Library/Logs/reentry-<run UUID>.log`, available through the existing log sharing UI. Duplicate observations and the fingerprint map are bounded. Diagnostic timing overhead remains a limitation, not silently called zero.
- `check_source.py` strips the marked DEBUG blocks and compares against `1f47275`, with negative tests for non-diagnostic changes and active measurement calls. The real packager stamps only its copied app, preserves its source, and ordinary repacks remove any inherited trace activation. The native collector tests compile a real no-op mutation before requiring its expected assertion failure; compilation failure is not an acceptable negative control.
- New workflow `ios15-reentry-device.yml` pins Xcode26.2/17C52 and the existing device native-dependency cache, runs collector tests, builds the actual iOS15 App and packages a distinctly named diagnostic IPA. It does not run the blocked App Simulator target and does not claim UIKit acceptance from the Foundation collector test. Cloud completion and artifact checks remain required before delivery.
- Reused the existing reviewer for parser implementation and source/packaging review. Parser fixtures were corrected to follow real callsite shapes (CV↔VM session links, cell generation/index, text/host parent links, oldH/newH writes); synthetic events pre-filled with session/index were not accepted as sufficient integration evidence. Hardware feedback loop and interpretation boundaries are documented in `scripts/reentry-diagnostics/README.md`.

## 2026-09-23 — full-source device baseline probe, not another readiness patch

- Restarted from verified `2f21e24`; rejected functional candidate `272656b` is not carried into this branch. The old untracked Simulator draft is recoverably archived outside the repo, including its sliced text-view class and invalid report/fixture contract.
- New `DeviceProbe.swift` uses the complete production `SelectableMarkdownView` parser/render path and its actual text layout/correction/attachment lifecycle, plus original host/cell/layout classes. A disposable full App source copy changes only the Debug entrypoint and adds trace-read helpers. Core rendering sources remain byte-identical and inverse restoration of the overlay is checked.
- The package is an independent `com.openminis.layoutprobe` test App, with no normal Minis URL/document/extension registration or normal app-group/keychain entitlements. Normal Minis and private conversation data are not modified. The new app runs neutral fixtures on a real iOS15 device and shares a run-UUID-scoped JSON.
- Original production events, not delayed snapshots, feed the verdict. The report validator now requires actual preference→delivery→cell acceptance→return→cache association and matching configure/text ownership; same-owner but different-row recovery is rejected. Missing records and a backwards clock are INVALID. A recovered terminal snapshot does not erase a short transient from the result.
- Fresh local evidence: report tests had seven failing checks before validator repair, now 22/22 pass; build-overlay/bundle-isolation tooling 3/3; original source-isolation 4/4; Swift grammar and YAML/all embedded Bash syntax checks pass. These validate tooling and syntax, NOT native runtime behavior.
- Dedicated workflow `ios15-provisional-height-device.yml` retains Xcode26.2/17C52, SDK26.2 and the established device dependencies. Original observation-only checks run before the reversible test overlay. Full App compilation/package validation is separate from actual phone execution, which remains NOT_RUN until the test App is launched and its report is validated.

## 2026-09-23 — first actual-device report valid but target stimulus was not exercised

- Run35805118764 / fa407d11 full-source device probe compiled and passed package verification, and its separate IPA was delivered. The returned iOS15.1.1 report contains 645 intact events over 3.198s, all six cases, no cap/error, and valid final controls. Initial short text settled at246, actual code growth429, plain text147 at404pt text width. This is successful execution of the baseline test facility, not successful reproduction or a fix.
- The report remains INCONCLUSIVE. Initial rendering briefly proposed174.667 then correctly delivered246. During deferred re-entry and settle there were no text-size events; the same already-measured cell/text tree was retained. Ordinary remove/reinsert did not reach the intended first-unmeasured-text state. Private input and report remain outside the repo; local evidence is `reports/openminis-ios15/device-report-eef53fc7/`.
- Narrow test-driver correction: only the target phase gets a new UIKit reuse identifier, forcing UIKit to construct a fresh genuine SelfSizingCell/host/text tree with the same fixture and viewport. This does not modify any production rendering or layout implementation. The regular reuse control is retained.
- Added text identity to snapshots and a requiresFreshReentry report precondition. Its reused-tree negative case failed before the validator change and passes afterwards; missing identities and malformed flags are INVALID. Old fa407d1 reports without the new precondition retain their original INCONCLUSIVE classification. Report tests26/26, overlay/bundle tests3/3, original isolation4/4 and Swift grammar/diff checks pass. New native execution still required.

## 2026-09-23 — fresh-view result remains inconclusive; move capture to actual list context

- Run35827132483/5d848977 compiled, verified and delivered. The second actual iOS15.1.1 report (SHA4767ed7b089469a5c8181eece25f3d216a1cfa4d0ef2f41340b10e327c17dc13) has656 events, valid controls and distinct target cell/text IDs. Target137/139 unset174.667,140/142 finite246,143 preference246,145 delivery246,148 return246. No wrong cache write: INCONCLUSIVE, not a fix and no longer a missing fresh-tree trigger.
- Original-App capture had actual tracking/deceleration,255 items and viewport801; one short row/policy flag is not equivalent. Read-only review confirmed real wrapper/model/cache/Coordinator differences; main rejected its unsupported claim that unset width occurs only in the uncached path. FixedSize exists on both sides.
- Plan: `docs/ios15-real-list-replay-plan.md`. New test entry imports existing private clean export locally, uses actual RawMessage.toChatMessage (including strip/split/reasoning) and consecutive assistant folding, caches through actual VM method, displays actual CollectionViewMessageListV3. A simulator-only import was rejected because it drops reasoning and mishandles this export's string tool input/assistant grouping. The clean export omits media bytes and tool results; these limits are explicit, no success/result is fabricated.
- Actual gesture capture is one-shot, idle-paused, bounded20s plus unchanged8000-event cap; no reset/bypass, no fake gestures, no direct sizing setters in replay controller. Only passive snapshots and hashed text are exported. New capture validator11 tests plus historical26/overlay3/isolation4 pass locally. Two Swift files parse; YAML/bash syntax and diff checks pass. Cloud Foundation tests must verify real pause/start/finish/cap and reject a compiling no-op pause mutation; Apple App compilation and artifact verification are still required before delivery.
- Production `src/` unchanged. Source attestation expanded to actual model/cache/row files. No original/private input is staged or sent to CI. This is a more faithful diagnostic capture, not a production repair.

## 2026-09-23 — resumed real-list capture preflight

- Preserved the interrupted WIP on5d848977 and reran collection through supported script-directory discovery. Previous ModuleNotFoundError/missing-workdir results were invocation failures, not product regressions. This Linux host has no xcrun/swiftc; native execution remains a cloud/device gate.
- Fixed repeated pan/leave marker dedup with per-action ordinals. Native Foundation tests now exercise the actual extracted diagnostic callbacks and real collector, with a state-only recognizer substitute. Both no-op-pause and missing-ordinal mutations must compile successfully and then fail their behavioral assertions. The generator itself ran locally and all generated Swift parsed; compilation has not yet occurred at this checkpoint.
- Strengthened the replay validator after27 tests exposed15 real false-positive failures, then5 further boundary/CLI tests failed before their fixes. Final33 protocol cases include repeated gestures, two remounts, preserved transient zero-size samples, wrong producer/config links, changed model/text/viewport and mandatory external CLI provenance pins. Valid capture requires an actual production configure→text association and real viewport/deceleration evidence. CAPTURE_VALID still does not mean reproduced or repaired.
- Driver snapshots are passive and now include hashed model identity/content, actual Coordinator/VM/cell visibility, with natural bounded waiting only for remount presence (not height recovery). Removed the synchronous fake transition-suspend toggle. Warm-only motion without configure/text witnesses is INCOMPLETE; stopping mid-remount or mid-gesture is not success.
- Local evidence lives outside git in `reports/openminis-ios15/replay-wip-verification/`. Production src remains exactly2f21e24. Historical driver/report validator are retained; original private input is neither staged nor embedded. Cloud App compilation, native mutation results, artifact/source hashes, signing and independent bundle checks are still required before delivery.

## 2026-09-23 — first replay CI found a missing Foundation-test logging adapter

- Commit8ab609178ca6f673e1e977d986f887496f975a66 pushed only to fork fix/ios15-provisional-height. Run35853825264/job107157513862 failed in step4, before complete App compilation. Original source isolation4/4, actual collector behavior and compiling no-op collector mutation passed.
- Actual compiler error: generated RecorderWindow.swift:13 cannot find AppLogger in scope. The extracted real collector retains its shared singleton's AppLogger reference. The older collector executable supplies that non-rendering adapter in CollectorTests.swift, not in Recorder.swift; the new independent executable omitted it. Grammar checks cannot detect this missing symbol. Final independent review reached the same finding and explicitly corrected its earlier assumption that Recorder.swift was standalone.
- Minimal repair: add the same test-only AppLogger init/info adapter to CaptureWindowTests.swift. Do not link the entire old test file with its second @main; no production class/collector behavior is replaced. Both window mutants must still compile before their expected assertion failures count. Failed run retained under workspace reports/openminis-ios15/replay-8ab6091; no IPA came from it. Rebuild and artifact verification remain required.

## 2026-09-23 — first replay App device run crashed on attachment tiles

- Delivered Minis-LayoutProbe-5be66f70.ipa; user launched it on iOS15.1.1 and it terminated about16s after launch. The returned .ips (SHA2a512bc547c6be8d6618e3b0db354d074b5fe8129300281f9eef4dc902b1dc1d, evidence `reports/openminis-ios15/replay-report-fd6ffe17/`) is a Swift runtime trap, not a memory kill: EXC_BREAKPOINT/SIGTRAP via `_assertionFailure` in `AIChatViewModel.minisAppGroupRoot` → `minisSkillsPersistentDir` → `resolveMinisFileURL(Cached)` → `minisMediaCacheKey` → `AsyncImageTile.body`, during SwiftUI row layout sizeThatFits.
- Root cause is probe-only: the isolated bundle deliberately has no normal-Minis App Group, and the imported session contains user `<user-attached-files>` metadata, so rendering attachment tiles resolves a minis:// URL through the missing container and asserts. This is a test-harness defect, not evidence about the original layout bug.
- Driver fix: after the actual RawMessage.toChatMessage conversion, count then clear `message.attachments`/`inputAttachments`. Displayed text is unchanged (the XML is already stripped by the production converter); no tile renders, no App Group access; report limits now name the extra omission. Production source remains exactly2f21e24; rebuild/verification required before another device run.
