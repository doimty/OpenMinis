...
final class SpeechRecognitionManager: ObservableObject {
    ...
    var languageLabel: String {
        guard let langCode = locale.compatLanguageCode else {
    ...
    private init() {
        ...
        if let savedId = UserDefaults.standard.string(forKey: Self.savedLocaleKey),
           allSupported.contains(where: { $0.identifier == savedId }) {
            ...
            let systemLangCode = Locale.current.compatLanguageCode ?? "en"
            resolvedLocale = allSupported.first { $0.compatLanguageCode == systemLangCode }
        ...
        let preferredCodes = Locale.preferredLanguages.map { Locale(identifier: $0).compatLanguageCode ?? "" }

        let sorted = allSupported.sorted { a, b in
            let aCode = a.compatLanguageCode ?? ""
            let bCode = b.compatLanguageCode ?? ""
