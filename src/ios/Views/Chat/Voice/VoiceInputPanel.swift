...
enum VoiceLanguages {
    ...
    private static func bestSupportedMatch(for preferred: String, in supported: [String]) -> String? {
        ...
        if let lang = loc.compatLanguageCode {
            if let region = loc.compatRegionCode {
    ...
    static func option(for code: String) -> Option {
        ...
        let name = current.localizedString(forIdentifier: code)
            ?? loc.compatLanguageCode.map { current.localizedString(forLanguageCode: $0) ?? $0 }
            ?? code
        let primary = loc.compatLanguageCode ?? String(code.prefix(2))
