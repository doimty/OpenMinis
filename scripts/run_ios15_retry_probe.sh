...
build_and_run_probe() {
  local app="$1" main="$2" out="$3" data_dir="$4"
  mkdir -p "$out"
  OUT="$out" APP="$app" MAIN="$main" python3 - <<'PY'
import hashlib,json,os,plistlib,subprocess
from pathlib import Path
root,out,build=(Path(os.environ[k]) for k in ('ROOT','OUT','BUILD'))
app=os.environ['APP']; main=os.environ['MAIN']
inputs=[root/'src/ios/Shared/LegacyHostingContent.swift',root/Path(main)]
...
appdir=build/(app+'.app'); appdir.mkdir()
info={'CFBundleExecutable':app,'CFBundleIdentifier':'com.openminis.'+app,'CFBundleName':app,'CFBundlePackageType':'APPL','CFBundleVersion':'1','CFBundleShortVersionString':'1.0','MinimumOSVersion':'15.0','UIDeviceFamily':[1,2],'UILaunchScreen':{},'UISupportedInterfaceOrientations':['UIInterfaceOrientationPortrait']}
(appdir/'Info.plist').write_bytes(plistlib.dumps(info))
PY
  xcrun --sdk iphonesimulator swiftc -target "$ARCH-apple-ios15.0-simulator" \
    ...
    "$ROOT/$2" \
    -o "$BUILD/$1.app/$1" > "$out/compile.log" 2>&1 \
    || { cat "$out/compile.log"; exit 1; }
  codesign --sign - "$BUILD/$1.app"
  OUT="$out" APP="$1" DATA_DIR="$4" python3 - <<'PY'
import json,os,shutil,subprocess
from pathlib import Path
out=Path(os.environ['OUT']); build=Path(os.environ['BUILD'])
app=os.environ['APP']; data_dir=os.environ['DATA_DIR']
...
run('install',udid,str(build/f'{app}.app'))
command=['xcrun','simctl','launch','--console','--terminate-running-process',udid,'com.openminis.'+app]
try:
 ...
 raise SystemExit('FAIL: native probe timed out')
(out/'console.log').write_text(result.stdout)
data=Path(run('get_app_container',udid,'com.openminis.'+app,'data'))/('Documents/'+data_dir)
...
assert result.returncode==0 and report['passed'], report.get('details', report.get('conclusion','probe failed'))
print('PASS: native retry matrix (forced legacy host on iOS26.2)')
PY
}

build_and_run_probe RetryProbe scripts/ios15-retry-probe/ProbeApp.swift "$OUT/unit" retry-probe
build_and_run_probe HierarchyRetryProbe scripts/ios15-retry-probe/HierarchyProbeApp.swift "$OUT/hierarchy" retry-hierarchy-probe
